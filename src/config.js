...
const trimDate = newDate.toISOString().substr(0, newDate.toISOString().indexOf('.'));
const urlExt = "&$where=calldatetime>'" + trimDate + "'&$order=calldatetime desc";
const callsUrl = encodeURI(
  'https://data.baltimorecity.gov/resource/m8g9-abgb.geojson?$limit=50000' + urlExt
);
export const geoQueries = {
  liquorLicenses: {
    name: 'liquorLicenses',
    title: 'Liquor Licenses',
    url: 'https://data.baltimorecity.gov/resource/g2jf-x8pp.geojson?$limit=50000&licenseyear=2017',
    attributes: {
